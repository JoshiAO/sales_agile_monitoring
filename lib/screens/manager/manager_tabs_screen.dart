...
class ManagerTabsScreen extends StatefulWidget {
  const ManagerTabsScreen({super.key});

  @override
  State<ManagerTabsScreen> createState() => _ManagerTabsScreenState();
}

class _ManagerTabsScreenState extends State<ManagerTabsScreen> {
  int _currentIndex = 0;
  ...
  List<Widget> _buildPages() {
    return [
      SuperuserHomeScreen(key: ValueKey('mgr-home-$_refreshTick')),
      SuperUserDashboard(key: ValueKey('mgr-map-$_refreshTick')),
      SuperuserAgilePage(key: ValueKey('mgr-agile-$_refreshTick')),
      const FeedsPage(),
      AnnouncementPage(key: ValueKey('mgr-announcement-$_refreshTick')),
    ];
  }
